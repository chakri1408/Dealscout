...
from app.agents.deals import Deal, Opportunity
...
class AutonomousPlanningAgent(Agent):
    ...
    def notify_user_of_deal(
        ...
        if self.opportunity:
            ...
            self.message = self.messenger.notify(description, deal_price, estimated_true_value, url)
    ...
    notify_function = {
        ...
        "description": "Record an alert message for the user about the single most compelling deal; only call this one time",
        "parameters": {
            "type": "object",
            "properties": {
                "description": {
